...
def create_game():
    ...
    try:
        ...
        winning_team_players = (
            game.team1_players if game.winning_team == 1 else game.team2_players
        )
        losing_team_players = (
            game.team2_players if game.winning_team == 1 else game.team1_players
        )
        ...
        return (
            jsonify({"message": "Game recorded successfully", "game": game.to_dict()}),
            201,
        )
    ...
    try:
        ...
        if data.get("action") == "start":
            if len(tournament.players) != tournament.tournament_type:
                return (
                    jsonify(
                        {"error": f"Need exactly {tournament.tournament_type} players"}
                    ),
                    400,
                )
        ...
        if data.get("action") == "update_bracket":
            ...
            tournament.current_round = data.get(
...
def get_leaderboard():
    ...
    try:
        ...
        return jsonify({"leaderboard": leaderboard[:50]}), 200  # Top 50 players
...
def join_waitlist():
    ...
    return jsonify({"message": "Added to waitlist", "position": 1}), 200
