...
def register():
    try:
        ...
        if data.get("beach_member_since"):
            try:
                user.beach_member_since = datetime.strptime(
                    data["beach_member_since"], "%Y-%m-%d"
                ).date()
            except:
                pass
...
def login():
    try:
        ...
        return (
            jsonify(
                {"access_token": access_token, "user": user.to_dict(include_stats=True)}
            ),
            200,
        )
...
@auth_bp.route('/users/<target_user_id>/rsvps', methods=['GET'])
@jwt_required()
def get_user_rsvps(target_user_id):
    current_user_id = get_jwt_identity()
    ...
    # Users can get their own RSVPs. Admins can get anyone's.
    if current_user_id != target_user_id:
        current_requesting_user = User.query.get(current_user_id)
        if not current_requesting_user or not current_requesting_user.is_admin:
             return jsonify({'message': 'Unauthorized to view RSVPs for this user.'}), 403

    # Need to import RSVP model
    from app.models import RSVP
    rsvps = RSVP.query.filter_by(user_id=target_user_id).order_by(RSVP.updated_at.desc()).all()
